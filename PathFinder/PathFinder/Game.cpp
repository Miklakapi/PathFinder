...
void Game::run(RenderWindow& window) {
	...
	/////////
		...
		else if (type1 == Menu::Editor  && clickDelay.getElapsedTime().asSeconds() > 0.2) {
			delete menu;
			editor = new Editor;
			type = Type::InEditor;
		}
		else if (type1 == Menu::Description  && clickDelay.getElapsedTime().asSeconds() > 0.2) {
			description = new Description;
			type = Type::InDescription;
		}
	...
	/////////
		...
		if (editor->run(window) == Editor::Return) {
			type = Type::InMenu;
			delete editor;
			menu = new Menu;
		}
	...
	/////////
	/////////
	else if (type == Type::InDescription) {
		menu->run(window, false, sound);
		if (description->run(window, sound) == Description::Return) {
			delete description;
			type = Type::InMenu;
			clickDelay.restart();
		}
